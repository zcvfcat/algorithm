...
def insert_sort(array: list, start: int, end: int):
    for idx in range(start, end - 1, 1):
        # array[start] ~ array[idx] 까지 정렬 되어 있음
...
def tim_sort(array: list, unit=32):
    length = len(array)
    ...
    while round_size < length:

        for start in range(0, length, round_size * 2):
            mid = start + round_size - 1
            end = min(start + round_size * 2 - 1, length)
