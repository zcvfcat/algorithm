class Node:
    def __init__(self, key: int, value: str, M: int):
        self.key = key
        self.value = value
...
class MTree:
    ...
    def insert_node(self, key: int, value: str, node: Node):
        if key == node.key:
            node.value = value
            return
    ...
    def get_child_index(self, key: int, node: Node):
        return key % self.M
    ...
    def search_node(self, key: int, node: Node):
        if key == node.key:
            return node.value
